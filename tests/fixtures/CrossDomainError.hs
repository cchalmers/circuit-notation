...
-- @circuitS@ block: @acc + a + b@ mixes values sampled from a @domA@ bus and
...
-- pattern), so the marker sits on the @circuitS@ line.
...
crossDomainError = circuitS \(Signal a, Signal b) -> do  -- cross-domain-error-marker
