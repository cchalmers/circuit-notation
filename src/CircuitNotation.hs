...
-- | Which keyword marked a value boundary. @Signal@ asserts the bus /is/ a
-- 'Signal' (generating the concrete @SigTag@, which drives type inference);
-- @Fwd@ samples the forward channel of any signal-like bus (generating the
-- class-constrained @FwdTag@, which requires the bus type to be determined
-- by context). In ordinary (bus-level) @circuit@ blocks the two are
-- interchangeable.
data SigMarker = SignalMarker | FwdMarker
...
data PortDescription a
    ...
    -- ^ generated by @circuitV@: a value-boundary bus expression, tagged
...
-- | Recognise the value-boundary marker keywords (see 'SigMarker').
