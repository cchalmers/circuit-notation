...
isCircuitVVar :: p ~ GhcPs => HsExpr p -> Bool
isCircuitVVar = isSomeVar "circuitV"
...
data PortDescription a
    ...
    | FwdPat (LPat GhcPs)
    | SigTagExpr (LHsExpr GhcPs)
    -- ^ generated by @circuitV@: a value-boundary bus expression, tagged with
    -- 'SigTag' to pin the bus type to a signal
    | SigTagPat (LPat GhcPs)
    -- ^ generated by @circuitV@: a value-boundary bus pattern, tagged with
    -- 'SigTag' to pin the bus type to a signal
...
data CircuitState dec exp nm = CircuitState
    ...
    , _circuitVlaves  :: PortDescription nm
...
parseCircuit = \case
  ...
  L _ (simpleLambda -> Just ([matchPats], body)) -> do
    circuitVlaves .= bindSlave matchPats
    circuitBody body
...
checkCircuit = do
  ...
  circuitVlaves %= L.transform modifyMulticast
...
bindWithSuffix dflags dir = \case
  ...
  SigTagExpr (L _ _) -> nlWildPat
  SigTagPat lpat -> sigTagP lpat
...
expWithSuffix dir = \case
  ...
  SigTagExpr lexpr -> sigTagE lexpr
  ...
  SigTagPat (L l _) -> tagE $ varE l (trivialBwd ?nms)
...
sigTagP :: (p ~ GhcPs, ?nms :: ExternalNames) => LPat p -> LPat p
sigTagP a@(L l _) = L l (conPatIn (noLoc (signalTagName ?nms)) (prefixCon [a]))

sigTagE :: (p ~ GhcPs, ?nms :: ExternalNames) => LHsExpr p -> LHsExpr p
sigTagE a@(L l _) = varE l (signalTagName ?nms) `appE` a
...
replaceFwdPats prefix = L.transformM \case
  FwdPat lpat@(L loc _) -> do
    (i, pats) <- get
    put (i + 1, pats <> [lpat])
    pure (SigTagPat (varP loc (prefix <> show i)))
  p -> pure p
...
replaceFwdExprs prefix = L.transformM \case
  FwdExpr lexpr@(L loc _) -> do
    (i, exprs) <- get
    put (i + 1, exprs <> [lexpr])
    pure (SigTagExpr (varE loc (var (prefix <> show i))))
  p -> pure p

-- | The @circuitV@ (value-level circuit) version of 'circuitQQExpM'.
--
...
circuitVQQExpM = do
  ...
  if boundaryCount == 0 then circuitQQExpM else circuitVQQExpM'
...
-- [value-components]
-- The value-level bindings of a @circuitV@ block are split into the
...
-- a single circuitV block to span several clock domains: per-cycle values
...
-- @circuitV@ block; the @Int@ indexes into the respective collection.
...
transform debug = SYB.everywhereM (SYB.mkM transform') where
  ...
  transform' (L _ (HsApp _xapp (L _ circuitVar) lappB))
    | isCircuitVar circuitVar = runCircuitM $ do
        ...
        when debug $ ppr x
        pure x
    | isCircuitVVar circuitVar = runCircuitM $ do
        x <- parseCircuit lappB >> completeUnderscores >> circuitVQQExpM
        when debug $ ppr x
        pure x
  ...
  transform' (L _ (OpApp _xapp c@(L _ circuitVar) (L _ infixVar) appR))
    ...
    | isDollar infixVar && dollarChainIsCircuit "circuitV" circuitVar = do
        runCircuitM $ do
          x <- parseCircuit appR >> completeUnderscores >> circuitVQQExpM
          when debug $ ppr x
          pure (dollarChainReplaceCircuit "circuitV" x c)
...
data ExternalNames = ExternalNames
  ...
  -- a signal; used at the value boundary of @circuitV@ blocks
